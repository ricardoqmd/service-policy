 ...
 *
 * <p>Selected for a request when {@code resourceType} matches the resource type and the
 * request verb is in {@code actions} (or {@code actions} contains {@code "*"}). Rules are
 * combined with {@code combiningAlgorithm}; if no rule applies, {@code defaultEffect} is
 * used.
 *
